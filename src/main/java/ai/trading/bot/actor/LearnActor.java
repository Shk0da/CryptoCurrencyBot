...
import ai.trading.bot.domain.Status;
...
import ai.trading.bot.util.CSVUtil;
import ai.trading.bot.util.LSTMNetwork;
import ai.trading.bot.util.StockDataSetIterator;
...
import static ai.trading.bot.util.StockDataSetIterator.deNormalize;
import static ai.trading.bot.util.StockDataSetIterator.getVectorSize;
...
public class LearnActor extends UntypedAbstractActor {
    ...
    public LearnActor(StockMarket market, String instrument) {
        this.market = market;
        this.instrument = instrument;
        this.locationToSave = "NeuralNetwork" + market.name() + instrument;
    }
    ...
    public void preStart() {
        ...
        String predictionRepositoryName = market.name().toLowerCase() + "PredictionRepository";
        predictionRepository = (PredictionRepository) ApplicationContextProvider
                .getApplicationContext()
                .getBean(predictionRepositoryName);
        log.debug("{}::{} predictionRepositoryName has been init: {}::{}", market, instrument, predictionRepositoryName, predictionRepository);
    }
    ...
    private void predict() {
        ...
        List<Candle> last = candleRepository.getLastCandles(instrument, getVectorSize());

        // check vector
        if (last.size() < getVectorSize()) return;

        // check new data
        double vectorClose = last.get(last.size() - 1).getPrice();
        ...
        try {
            ...
            log.error("Predict {} failed: {}", instrument, ex.getMessage());
    ...
    private void trainNetwork() {
        ...
        List<Candle> candles = candleRepository.getLastCandles(instrument, candleRepository.getLimit());
        ...
        try {
            ...
            if (storeDisk && neuralNetwork != null) {
                try (DirectoryStream<Path> dirStream = Files.newDirectoryStream(Paths.get("."), locationToSave + "_*")) {
                    ...
                    log.error("Failed save to disk {}: {}", instrument, ex.getMessage());
                }
            }
        } catch (Exception ex) {
            log.error("Failed create network {}: {}", instrument, ex.getMessage());
            ex.printStackTrace();
        }
    }
